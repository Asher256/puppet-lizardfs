...
#
# 'lizardfs::ha::pacemaker': install and configure corosync and pacemaker for HA LizardFS.
#
...
#   pacemaker_quorum_members   => [ 'master.dev', 'shadow.dev' ],
# }
#
# === Parameters
#
# [*pacemaker_quorum_members*] a list of the Lizardfs masters IPs or FQDNs (must be resolvable by dns).
# Ex: pacemaker_quorum_members   => ['lizardfs-master.dev', 'lizardfs-shadow.dev'],
#
...
)
{
  ...
  if empty($pacemaker_quorum_members) {
      fail('The pacemaker_quorum_members parameter is required.')
  }
    ...
    class { 'corosync':
      ...
      #debug                     => true,
      set_votequorum             => true,
      quorum_members             => $pacemaker_quorum_members,
